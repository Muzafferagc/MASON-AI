...
### Eklendi (Faz B — Obsidian köprüsü: hafızanın iki yönlü markdown aynası)
- **Gerçek bir Obsidian vault'u (`MasonVault/`):** Hafıza, görevler ve planlar artık markdown notları olarak diske aynalanıyor — Obsidian'da **Open folder as vault** ile açıp beynini gerçek graph view'da gezebilirsin. Her bilgi bir not (`Hafıza/`), projeler merkez not (`Projeler/`, bilgiler `[[proje]]` bağlantısıyla bağlanır), görevler tek checkbox listesi (`Görevler.md`), planlar `Planlar/` klasöründe. `MASON.md` kullanım kılavuzu otomatik oluşur.
- **İki yönlü:** Obsidian'da notu düzenle (içerik, `kategori`, `proje`, `## Not`) → MASON geri okur (içerik değişince embedding yenilenir). `Görevler.md`'de **kutuyu işaretle → görev tamamlanır** (tekrar edenler sonraki tarihi açar), yeni `- [ ] başlık 📅 tarih` satırı → yeni görev, `Hafıza/`ya `mason_id`siz not bırak → hafızaya eklenir. Plan içeriği düzenlemesi de geri okunur.
- **Güvenlik — silme baypası yok:** Vault'tan dosya silmek hafızayı **silmez**; sonraki eşitlemede dosya geri yazılır. Silme yalnızca uygulama içinden (şifre korumasıyla) yapılır. Uygulamadan silinen kaydın notu vault'tan da kalkar.
- **Çakışma emniyeti:** İki taraf da aynı anda değiştiyse veritabanı kazanır; senin sürümün `... (çakışma).md` olarak yanına kopyalanır — hiçbir şey kaybolmaz.
- **Akıllı ve ucuz eşitleme:** Her dosya için çift özet (DB tarafı + disk tarafı, SHA-1) `obsidian_sync` tablosunda tutulur; yalnızca değişen dosyaya dokunulur. Arka planda ~60 sn'de bir (ayarlanabilir `obsidian_sync_interval_sec`), API maliyeti sıfır. Dosyayı yeniden adlandırabilirsin — kimlik frontmatter'daki `mason_id`de.
- **Arayüz:** Ayarlar'a **🔮 OBSIDIAN KÖPRÜSÜ** bölümü — aç/kapat, vault klasörü seçimi (boş = proje yanındaki `MasonVault`), **🔄 ŞİMDİ EŞİTLE** (sonuç özeti: dışarı/içeri/yeni/çakışma) ve **📂 VAULT'U AÇ** butonları, son eşitleme durumu.
- **Yeni:** `mason/obsidian.py` (render + frontmatter ayrıştırıcı + hash-tabanlı eşitleme motoru), `run.py` → `api.obsidian_sync_now / obsidian_status / open_vault` + `_obsidian_loop` arka plan iş parçacığı, config → `obsidian_enabled / obsidian_vault_path / obsidian_sync_interval_sec`. `MasonVault/` gitignore'da (kişisel veri). `test_core.py`'ye 19 Obsidian testi (toplam **157 doğrulama, hepsi geçiyor**).
